...
save("/home/yangbo/data/fixed-center.jld", "data", data)

p3 = plot([abs(sqrt((p.s.x-0)^2+(p.s.y-0)^2)-1.0)/1.0 for p in data], label="distance relative error")
plot(p3,size = (1400, 1400),xtickfontsize=50,ytickfontsize=50,legendfontsize=50,width=5)

p3 = plot([p.s.ψ for p in data], label="ψ")
plot(p3,size = (1400, 1400),xtickfontsize=25,ytickfontsize=25,legendfontsize=50,width=5)
